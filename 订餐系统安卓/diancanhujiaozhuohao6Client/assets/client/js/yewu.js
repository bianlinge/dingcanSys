...
//设置类别列表
    ...
    p4.tpl = '<li><a href="#" onclick="billDetail(%s);">'+
        ...
        '</a></li>';
    p4.colums = ["id","ndate","gnames","total","address"];
...
function tijiaouser(){
    ...
    bill.note = note;
    bill.statecn="未付款";
    bill.todate = $("#todate").val();
    bill.totime = $("#totime").val();
...
function mybillslist(type){
    if(userinfo.roletype=="2"){
        ajaxCallback("mybills",{},function(data){
            billlist = data;
...
function billDetail(id) {
    ...
    $("#bfenqi").text("到店日期:"+bill.todate+",时间:"+bill.totime);
