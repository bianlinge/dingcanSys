...
import com.ideabobo.service.FuwuService;
import com.ideabobo.util.Page;

@Service
@Transactional
public class FuwuServiceImp implements FuwuService {
	...
	public void delete(Integer uuid) {
		sessionFactory.getCurrentSession().delete(
				sessionFactory.getCurrentSession().load(Fuwu.class, uuid));
	}

	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public Fuwu find(String uuid) {
		return (Fuwu) sessionFactory.getCurrentSession().get(Fuwu.class, Integer.parseInt(uuid));
	...
	public Fuwu find(Fuwu model) {
		try {
			List<Fuwu> list = getHibernateTemplate().findByExample(model);
			if (list.size() > 0) {
				return list.get(0);
			}
	...
	public List<Fuwu> list() {
		return sessionFactory.getCurrentSession().createQuery("from Fuwu").list();
	}
	...
	public Page findByPage(Page page, Map paramsMap) {
		...
		sb.append("from Fuwu u where 1=1");
		if (paramsMap.get("fuwuname") != null&& !"".equals(paramsMap.get("fuwuname"))) {
			sb.append(" and u.fuwuname like '%" + paramsMap.get("fuwuname")+ "%'");
		}
